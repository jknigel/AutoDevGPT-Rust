...
impl ManagingAgent {
    ...
    pub async fn execute_project(&mut self) {
        ...
        for agent in &mut self.agents {
            match agent.execute(&mut self.factsheet).await {
                Ok(_) => {},
                Err(e) => {
                    dbg!(e);
                }
...
mod tests {
    ...
    async fn tests_managing_agent() {
        let usr_request: &str = "Need a full stack app that fetches and tracks my fitness progress. Needs to include my workout and some data.";
